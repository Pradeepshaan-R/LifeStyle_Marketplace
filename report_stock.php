...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->Cell(277, 5, 'Stock Report (' . date('Y-m-d') . ')', 0, 1, 'C'); // Title
        $this->SetFont('Times', '', 13);
        $this->Cell(276, 10, 'Stock Details of SenzAgro (Pvt)Ltd.', 0, 0, 'C');
    ...
    function TableHeader() {
        ...
        $this->Cell(60, 10, 'SUPPLIER', 1, 0, 'C', true);
        $this->Cell(60, 10, 'MANUFACTURE', 1, 0, 'C', true);
        $this->Cell(60, 10, 'EXPIRE', 1, 1, 'C', true);
    }
    ...
    function TableBody($conn) {
        ...
        $count_stock = 1;
        $stmt = $conn->query("SELECT * FROM stock AS st JOIN product AS pro ON st.product_product_id = pro.product_id JOIN supplier ON st.supplier_supplier_id = supplier.supplier_id");
        while ($row = $stmt->fetch_assoc()) {
            $this->Cell(15, 10, $count_stock++, 1, 0, 'C', true);
            $this->Cell(60, 10, $row['product_name'] . ' ' . $row['supplier_lname'], 1, 0, 'L', true);
            $this->Cell(60, 10, $row['supplier_uname'], 1, 0, 'C', true);
            $this->Cell(60, 10, $row['stock_reg_date'], 1, 0, 'C', true);
            $this->Cell(60, 10, $row['stock_expire_date'], 1, 1, 'C', true);
        }
    }
